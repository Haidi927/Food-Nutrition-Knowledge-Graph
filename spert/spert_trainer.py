...
class SpERTTrainer(BaseTrainer):
    ...
    def _load_model(self, input_reader):
        ...
        # 使用通用 AutoConfig 加载配置
        config = AutoConfig.from_pretrained(self._args.model_path, cache_dir=self._args.cache_path)
        util.check_version(config, model_class, self._args.model_path)

        config.spert_version = model_class.VERSION

        # 原始 transformer encoder 加载（BERT）
        base_model = AutoModel.from_pretrained(self._args.model_path, config=config, cache_dir=self._args.cache_path)

        # 构造 LoRA config
        peft_config = LoraConfig(
            task_type=TaskType.FEATURE_EXTRACTION,   # 如果你是 span-based NER/RE 就用 TOKEN_CLS
            r=8,
            lora_alpha=32,
            lora_dropout=0.1,
            bias="none",
        ...
        # 用 LoRA 包装 base_model
        encoder_with_lora = get_peft_model(base_model, peft_config)

        # 打印 LoRA 结构（可选）
        encoder_with_lora.print_trainable_parameters()

        # 加载 SpERT 模型，同时注入 encoder_with_lora
        model = model_class(
            config=config,
            cls_token=self._tokenizer.convert_tokens_to_ids('[CLS]'),
            relation_types=input_reader.relation_type_count - 1,
            ...
            size_embedding=self._args.size_embedding,
            freeze_transformer=self._args.freeze_transformer,
            encoder=encoder_with_lora   # ✅ 注入 LoRA encoder
        )
        ...
        for batch in tqdm(data_loader, total=total, desc='Train epoch %s' % epoch):
            try:
                ...
                if 'out of memory' in str(e):
                    print(f"[OOM] Step {iteration} of epoch {epoch}: 显存不足，跳过该 batch")
