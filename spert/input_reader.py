...
class BaseInputReader(ABC):
    def __init__(self, types_path: str, tokenizer: BertTokenizer, neg_entity_count: int = None,
        ...
        types = json.load(open(types_path,"r", encoding="utf-8"), object_pairs_hook=OrderedDict)  # entity + relation types
...
class JsonInputReader(BaseInputReader):
    ...
    def _parse_entities(self, jentities, doc_tokens, dataset) -> List[Entity]:
        ...
        for entity_idx, jentity in enumerate(jentities):
            entity_type = self._entity_types.get(jentity['type'])
            if entity_type is None:
                # 如果没找到，就用默认类型，比如 'None'
                entity_type = self._entity_types['None']

            start, end = jentity['start'], jentity['end']

            # ⚡ 跳过非法实体：越界 / 空 span
            if not (0 <= start < end <= len(doc_tokens)):
                continue
    ...
    def _parse_relations(self, jrelations, entities, dataset) -> List[Relation]:
        ...
        for jrelation in jrelations:
            relation_type = self._relation_types.get(jrelation['type'])
            if relation_type is None:
                relation_type = self._relation_types['None']

            head_idx = jrelation['head']
            tail_idx = jrelation['tail']

            # ⚡ 跳过非法关系：索引越界 / head==tail
...
def _parse_tokens(jtokens, dataset, tokenizer):
    ...
    for i, token_phrase in enumerate(jtokens):
        # 如果再加下去会超过 max_len-1，就直接停
        if len(doc_encoding) >= max_len - 1:
            break
        ...
        # 如果当前 token 加进去会超长，就截断
        if len(doc_encoding) + len(token_encoding) >= max_len - 1:
            token_encoding = token_encoding[: (max_len - 1 - len(doc_encoding))]
